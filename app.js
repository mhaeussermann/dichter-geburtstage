...
request.onload = function() {
  const birthdayPoets = request.response;
...
for (let i = 0; i < poets.length; i++) {
    ...
    myPara1.textContent = 'Birthday: ' + moment(poets[i]['birth'].value).format('Do MMMM YYYY') + ' (' + moment(poets[i]['birth'].value).from() + ').';
    myPara2.textContent = 'Birthplace: ' + poets[i]['placeLabel'].value + '.';
    myPara3.textContent = 'Deathday: ' + moment(poets[i]['death'].value).format('Do MMMM YYYY') + '.';
